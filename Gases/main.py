...
class Simulation(object):
    ...
    def _generate_initial_positions(self, n, distrib="uniform"):
    ...
    def _generate_initial_velocities(self, n, v_mean, v_std, distrib="normal"):
        self.v_mean = v_mean
        self.v_std = v_std
        ...
        if distrib == "normal":
            ...
            _vel = np.array([[np.cos(theta) * v_mean, np.sin(theta) * v_mean] for i in range(n)])
    ...
    def add_molecules(self, molecule, n, v_mean, v_std, pos_distrib="uniform", v_distrib="normal", color="black"):
        # Generate initial positions and velocities
        self._generate_initial_positions(n, distrib=pos_distrib)
        self._generate_initial_velocities(n, v_mean, v_std, distrib=v_distrib)
        
        # Initialize objects of class Molecule in a list (set mass to 1 as default)
        _add_list = [Molecule(molecule,
                              position=self._positions[i, :],
                              velocity=self._velocities[i, :],
                              color=color,
                              mass=5.33e-26) for i in range(n)]
    ...
    def create_2D_box(self):
        fig = plt.figure(figsize=(9, 9*self.box_dim[1]/self.box_dim[0]), dpi=300)
        return fig
    ...
    def show_molecules(self, i):
        ...
        plt.plot(self.x_dynamics[0, 0, :i+1], self.x_dynamics[0, 1, :i+1],
                 color="red", linewidth=1., linestyle="-")
        
        # Plot a single molecule in red that is being tracked
        plt.scatter(self.x_dynamics[0, 0, i], self.x_dynamics[0, 1, i],
                    color="red", s=20)
        
        # Plot the rest of the molecules
        plt.scatter(self.x_dynamics[1:, 0, i], self.x_dynamics[1:, 1, i],
                    color=self.colors[1:], s=20)
    ...
    def run_simulation(self, max_time):
        ...
        for i in range(self.n_iters):                   
            ...
            mean_E = np.mean(self.masses * np.square(self.velocities_norm) / 2)
            boltzman = 1.38e-23
            _T = mean_E / boltzman

            print("Iteration:{0:d}\tTime:{1:.2E}\tV_RMS:{2:.2E}\tWall Pressure:{3}, Temperature in С:{4}".format(i, i*self.t_step, self.v_rms, _P, _T - 273))
    ...
    def make_animation(self, filename="animation"):
        ...
        anim = FuncAnimation(fig, self.show_molecules, frames=self.n_iters,
    ...
    def write_results(self):
        ...
        # with open("results/{0}.txt".format(self.name)) as f:
            print("KE={0:.4f} J".format(self.KE_sum))
            # print("T={0:.4f} K".format(self.T))
            print("P={0:.4f} Pa".format(self.P))
            print("V={0:.4f} m3".format(self.V))
            print("N={0:d}".format(self.n_molecules))
            # print("PV/NT={0:.4f}".format(self.P*self.V/(self.T * self.n_molecules)))
            print("Time:{0:.4f} s".format(self.max_time))
            print("Iterations:{0:d}".format(self.n_iters))
            print("Timestep:{0:.4f} s".format(self.t_step))


if __name__ == "__main__":
    # Create simulation object and define input parameters
    sim = Simulation(name="kinetic_theory_simulation", box_dim=[1.0, 1.0], t_step=2e-5, particle_radius=1e-2)
    
    # Add N2 molecules to the box
    sim.add_molecules("N2", n=1000, v_mean=400.0, v_std=10, v_distrib="normal")

    # Run the simulation and store the pressure output in P
    P = sim.run_simulation(0.1)
